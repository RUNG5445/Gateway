...
int readConfig(const char *fn, struct espGwayConfig *c) {
	...
	if (!SPIFFS.exists(fn)) {
		...
		Serial.print(fn);
		Serial.println(F(" does not exist .. Formatting65656565656565"));
#endif

Serial.println(F(" doessldlaskd;askd;kasl;dkasl;kdl;askdl;askd;lkasl;dkasl;kd;aslkd;laskdl;ask not exist .. Formatting"));
		SPIFFS.format();
		initConfig(c);
		return(-1);
	}
