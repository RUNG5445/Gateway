...
const char *server = "rung.ddns.net";
const int port = 8050;
...
#define dio0 13
String apiKeys[] = {
    "pk.2014fa207d7d8a965c04592d5c62bfba",
    "pk.2014fa207d7d8a965c04592d5c62bfba",
    "pk.011c6f4ddd11a57e6ca821b18f70ed95",
    "pk.b74d5f5303eb387253fbc2569350ef5"};
...
// Define the number of keys in the array
int numKeys = 4;
...
void sendLocationRequestwifi()
{
  ...
  if (!WiFi.isConnected())
  {
    SerialMon.println("Error: Not connected to Wi-Fi!");
    return;
  }

  String payload = "{\"token\":\"" + getRandomApiKey() + "\",\"radio\":\"lte\",\"mcc\":" + networkinfo.mcc + ",\"mnc\":" + networkinfo.mnc + ",\"cells\":[{\"lac\":" + networkinfo.lac + ",\"cid\":" + networkinfo.cid + ",\"psc\":0}],\"address\":1}";
  String response;
  ...
  SerialMon.println("Sending HTTP request:");
  ...
  while (wifiClient.connected() && (millis() - startTime) < TIMEOUT_DURATION)
  {
    if (wifiClient.available())
    {
      ...
      SerialMon.println(responseLine);
    }
  }
...
String fetchActiveNodeWIFI()
{
  ...
  Serial.println("\nReached maximum number of retries. Exiting fetchActiveNode()\n");
  return "";
}
  ...
  if (packetSize)
  {
    ...
    if (enddevices_num == 0)
    {
      ...
      for (int i = 0; i < 4; i++)
      {
        ...
        LoRa.endPacket();
        Serial.print("Packet sent. Packet Size: ");
